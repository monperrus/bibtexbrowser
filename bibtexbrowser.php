...
@define('LAYOUT','table'); // MG: may be table/list/deflist. defines the HTML rendering options (<table>, <ol>, <dl>, resp.). 'list' only works with 'ABBRV_TYPE'='default'.
...
class BibEntry {
  ...
  /** MG: replace [$ext] with an icon whose url is defined in a string
   *  e.g. getIconOrTxt('pdf') will show the icon defined by @string{icon_pdf="http://mywebsite/pdf.png"} or print '[pdf]'
   *  or   getIconOrTxt('pdf','paper') will show the icon defined by @string{icon_pdf="http://mywebsite/pdf.png"} or print '[paper]'
  ...
  /** MG: Read the url specified as the bibtex entry key $bibentry and return the link with icon or text
   ...
   * getLink($bibentry,$icon,$def) specifies the icon key $icon and default text $def.
  */
  function getLink($bibentry,$icon=NULL,$def=NULL) {
    if ($icon==NULL) { $icon=$bibentry; }
    $str = $this->getIconOrTxt($icon,$def);
    if ($this->hasField($bibentry)) {
       return ' <a'.(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'').' href="'.$this->getField($bibentry).'">'.$str.'</a>';
    }
    return '';
  }

  /** MG: DOI are a special kind of links, where the url depends on the doi */
  function getDoiLink() {
  ...
  /** MG: GS are a special kind of links, where the url depends on the google scholar id */
  function getGSLink() {
  ...
  /** returns a "[url]" link if relevant */  /* MG: modified to exploit the new method, while keeping backward compatibility */ 
  function getUrlLink() {
  ...
  */
  ...
  /** MG: Outputs an LI line (<li>) with spans for each element, using the value attribute the abbrev. (does not work with x-abbrv) */
  function toLI() {
  ...
  /** MG: Outputs an DL line (<dt>+<dd>) */
  function toDD() {
        echo '<dl class="bibline"><dt class="bibref"><a name="'.$this->getId().'">'.$this->getAbbrv().'</a></dt><dd class="bibitem">';
        echo bib2html($this);
        echo "</dd></dl>\n";
  }
...
function query2title(&$query) {
    if (isset($query[Q_BIBLIOGRAPHY])) return 'Publications in ' . $_SERVER['PHP_SELF'];
    $headers = array();
 ...
 $result[] = $bib;
  ...
  function multisearch($query) {
    ...
    foreach ($this->bibdb as $bib) {
        ...
        if ($entryisselected)  $result[$akey] = $bib;
  }

  if (array_key_exists( Q_BIBLIOGRAPHY, $query )) {
    ksort($result); // a bibliography is sorted in ascending order
  } else {
    krsort($result);
  }
...
class Dispatcher {
  ...
  function layout() { print $_GET[LAYOUT]; $this->query[LAYOUT]=$_GET[LAYOUT]; } // added by MG
